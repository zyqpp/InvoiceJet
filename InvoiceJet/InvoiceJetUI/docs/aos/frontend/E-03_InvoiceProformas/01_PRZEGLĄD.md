...
├──────────────────┬───────────────────────────────────────────────────────┤
│ [Menu boczne]    │ Invoice Proformas              [ + New Proforma ][⋮]  │
│ Documents        ├───────────────────────────────────────────────────────┤
│  - Invoices      │ [Pole Search                                   x]     │
│  - Proformas ←   ├───────────────────────────────────────────────────────┤
│  - Stornos       │ Grid Invoice Proformas                                │
│                  │ Document Number | Client | Issue | Due | Total | Stat │
...
Ekran składa się z paska tytułu, pola Search, gridu proform i paginacji. Pasek tytułu zawiera nazwę `Invoice Proformas`, przycisk `New Proforma` i menu kontekstowe z operacją `Delete selected`.

Kliknięcie wiersza gridu nawiguje do ekranu edycji proformy.
...
| Sekcja | Typ | Opis |
|---|---|---|
| Pasek tytułu | `div.app-header` | Wyświetla tytuł, przycisk `New Proforma` i menu kontekstowe. |
| Sekcja filtrów | `mat-form-field` | Pole `Search` filtruje dane przez `MatTableDataSource.filter`. |
| Grid proform | `table mat-table` | Wyświetla dokumenty w kolumnach `select`, `documentNumber`, `clientName`, `issueDate`, `dueDate`, `totalValue`, `documentStatus`. |
...
| Element | Typ | Funkcja |
|---|---|---|
| Przycisk New Proforma | `mat-raised-button` | Nawiguje do `/dashboard/add-invoice-proforma`. |
...
| Wiersz gridu | `tr mat-row` | Nawiguje do `/dashboard/edit-invoice-proforma/:id`. |

---

## 4. Scenariusz główny

1. Użytkownik przechodzi do ekranu `/dashboard/invoice-proformas`.
2. `AuthGuard` dopuszcza dostęp, jeżeli `AuthService.isLoggedIn()` zwraca `true`.
3. `InvoiceProformasComponent.ngOnInit()` wywołuje `loadInvoices()`.
4. `loadInvoices()` wywołuje `DocumentService.getDocuments(2)`.
5. Odpowiedź typu `IDocumentTableRecord[]` jest przypisywana do `dataSource.data` i `invoices`.
6. Grid wyświetla rekordy proform.
7. Ekran umożliwia filtrowanie, sortowanie, paginację, zaznaczanie, przejście do dodawania, przejście do edycji i usuwanie zaznaczonych proform.
...
| Dane załadowane | `dataSource.data` zawiera tablicę proform. | Sukces `DocumentService.getDocuments(2)`. |
...
| Nawigacja dodawania | Router przechodzi do `dashboard/add-invoice-proforma`. | Kliknięcie `New Proforma`. |
| Nawigacja edycji | Router przechodzi do `/dashboard/edit-invoice-proforma/{id}`. | Kliknięcie wiersza gridu. |
