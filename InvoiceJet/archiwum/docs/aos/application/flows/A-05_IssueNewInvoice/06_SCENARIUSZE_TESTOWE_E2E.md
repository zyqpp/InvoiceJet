...
| 3 | Formularz pobiera dane pomocnicze. | API wywołuje `GET /api/Document/GetDocumentAutofillInfo/1`. |
...
| 6 | Tester wybiera produkt i ustawia ilość. | UI przelicza `totalPrice`. |
| 7 | Tester klika `Issue`. | Frontend wysyła `POST /api/Document/AddDocument`. |
...
| 2 | Tester zostawia puste pole klienta. | `invoiceForm.invalid` ma wartość `true`. |
| 3 | Tester klika `Issue`. | `onSubmit()` kończy działanie bez wywołania API. |

## 3. Scenariusz negatywny: brak aktywnego konta bankowego

| Krok | Akcja testera | Oczekiwany wynik |
|---|---|---|
| 1 | Tester wypełnia poprawny formularz. | Formularz jest `valid`. |
| 2 | Tester klika `Issue`. | Frontend wysyła `POST /api/Document/AddDocument`. |
| 3 | Backend nie znajduje aktywnego konta. | `DocumentService.AddDocument()` rzuca `NoBankAccountAddedException`. |
| 4 | UI otrzymuje błąd HTTP. | Komunikat zalezy od konfiguracji `ErrorInterceptor`. |

## 4. Scenariusz: podgląd PDF w edycji

| Krok | Akcja testera | Oczekiwany wynik |
|---|---|---|
| 1 | Tester otwiera `/dashboard/edit-invoice/:id`. | Formularz ładuje dokument przez `GET /api/Document/GetDocumentById/{documentId}`. |
| 2 | Tester klika `Preview`. | Frontend wysyła `POST /api/Document/GetInvoicePdfStream`. |
...
|---|---|
| Klient | Rekord `Firm` powiązany z użytkownikiem przez `UserFirm.IsClient = true`. |
| Seria | Rekord `DocumentSeries` dla `DocumentTypeId = 1`. |
| Produkt | Rekord `Product` powiązany z aktywna firma. |
